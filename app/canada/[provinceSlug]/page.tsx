...
export function generateStaticParams() {
  return getProvinceSlugs().map((provinceSlug) => ({ provinceSlug }));
}
...
export default async function CanadaProvincePage({
  ...
  const { provinceSlug } = await params;
  const valid = new Set(getProvinceSlugs());
  if (!valid.has(provinceSlug)) {
    notFound();
  }
  ...
  const withRating = facilities.filter((f) => f.rating != null);
  const averageRating =
    withRating.length > 0
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Canada", item: `${SITE_URL}/canada` },
      {
        ...
        position: 3,
        name: provinceName,
        item: `${SITE_URL}/canada/${provinceSlug}`,
      },
    ],
  };
    ...
    <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      >
        ...
        <h1 className="mt-2 font-heading text-3xl font-semibold sm:text-4xl">
          Daycare, Preschool & Childcare in {provinceName}
        </h1>
        <p className="mt-3 max-w-2xl text-sm text-white/90">
          ...
          {cityPhrase ? (
            <>
              {" "}
              From {cityPhrase}, families have access to licensed options including {careTypePhrase}.
            </>
          ) : (
            <> Families have access to licensed options including {careTypePhrase}.</>
          )}{" "}
      ...
      <section className="mt-8 space-y-4 rounded-2xl border-2 border-teal-200/80 bg-teal-50/30 p-6">
        ...
        {cities.length === 0 ? (
          <p className="text-sm text-slate-600">
            We don&apos;t have facilities listed for {provinceName} yet. As new data becomes
            available, cities and facilities will appear here.
          </p>
        ) : (
